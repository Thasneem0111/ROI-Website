...
// API endpoint for AJAX update industry. Returns JSON.
...
if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if (!move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
        ob_clean();
        echo json_encode(['success'=>false,'message'=>'Error uploading image']); exit;
    }
}

if ($newImage) {
    $stmt = $conn->prepare("UPDATE industry SET `title` = ?, `description` = ?, `image` = ? WHERE id = ? LIMIT 1");
    $stmt->bind_param('sssi', $name, $description, $newImage, $id);
} else {
    $stmt = $conn->prepare("UPDATE industry SET `title` = ?, `description` = ? WHERE id = ? LIMIT 1");
