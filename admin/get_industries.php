<?php
// get_industries.php
// Return JSON list of industries for public site and admin

// Allow CORS for local dev / same-origin usage
...
// Fetch rows with a generic select then map columns in PHP to avoid SQL errors
$out = [];
...
if ($res) {
    while ($row = $res->fetch_assoc()) {
        ...
        // pick name from possible columns
        $name = null;
        if (isset($row['name'])) $name = $row['name'];
        elseif (isset($row['title'])) $name = $row['title'];
        elseif (isset($row['headline'])) $name = $row['headline'];

        // pick createdAt from possible columns
        $createdAt = null;
