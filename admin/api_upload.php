...
// Accepts FormData (name, description, image) and inserts into `industry` table.
// Returns JSON. Requires admin session.
...
// Detect which column the table uses (name vs title)
...
$useName = column_exists($conn, 'industry', 'name');
$useTitle = !$useName && column_exists($conn, 'industry', 'title');

if ($useName) {
    $insertSql = "INSERT INTO industry (`name`, `description`, `image`, `createdAt`) VALUES (?, ?, ?, NOW())";
} elseif ($useTitle) {
    $insertSql = "INSERT INTO industry (`title`, `description`, `image`, `created_at`) VALUES (?, ?, ?, NOW())";
} else {
    // fallback to generic insert with available columns
    $insertSql = "INSERT INTO industry (`description`, `image`) VALUES (?, ?)";
}
